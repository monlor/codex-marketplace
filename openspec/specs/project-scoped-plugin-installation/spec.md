...
Each plugin SHALL document its required host dependencies, expected activation steps, and a validation method so users can understand what remains outside the plugin boundary.
...
- **THEN** the documentation lists any required host binaries, runtime tools, or network assumptions
